...
const interviewDetailsSchema = z.object({
    ...
    time: z.string().regex(/^\d{2}:\d{2}$/, "Invalid time format (HH:MM expected)"),
...
export const createApplicationSchema = z.object({
    body: z.object({
        ...
        companyPhoneNumber: z.string().min(1, "Phone number is required").optional(),
        jobPortal: z.string().min(1, "Job portal is required").optional(),
        address: z.string().min(1, "Address is required").optional(),
        jobType: z.enum(["remote", "onsite", "hybrid"]),
        status: z.enum(["Applied", "Interview Scheduled", "Rejected", "Under Review"]),
        appliedDate: z.string().refine((val) => !isNaN(Date.parse(val)), "Invalid date format").optional(),
